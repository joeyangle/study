...
def make_sure(name,password,address):
    ...
    '''
    f1 = open(address,'r')
    f = f1.readlines()#此处应优化为for line in f1以减少内存占用，提高效率
    f1.close()
    for line in f:
        out_list = []
        line = line.split()
        if len(line)==0 :#采用len方法来判断是否为空，避免报错
            continue
        #print(line)
        if line[0] == name and line[1] == password:
            out_list.append(True)
            try: #try方法来避免报错，同时起到判断作用
                salary = line[2]
            except:
                salary = None
            if salary is None:
            ...
            out_list.append(None)
        return out_list#函数定义的太过于复杂，需要减少
...
def chat_out(address,p = 'f'):
    ...
    :return: 货物信息
    '''
    f = open(address, 'r',encoding='utf-8').readlines()#同上，需要改变以减少内存占用
    goods_out = []
    for line in f:
        if len(line) == 0:
            continue
        eachone = line.split()#split方法对文件进行列表化输出
        if p == 't':#加入了默认参数以表示是否print
            print(eachone)
...
def writefile(chart,salary,address1,address2):
    ...
    for line in change.readlines():
        line = line.split()
        if len(line) == 0:#加入判断，不然报错，列表取多会报错
            continue
    ...
    if choose == '2':
        ...
        if make_sure_out[0] == True:
            ...
            while True:
                ...
                elif c_price_add == '2':#修改价格，后面对用户输入判断没有跟上，误操作会报错
